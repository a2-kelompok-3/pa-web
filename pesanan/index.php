...
$user_id = $_SESSION['id'];
$orders = $mysqli->query("SELECT order.id, order.total_price, order.status,
user.username FROM `order` INNER JOIN user ON order.user_id=user.id WHERE
  ...
  <body>
        ...
        <div class="container" data-aos="fade-up">
          ...
          <table class="table" id="orderTable">
            ...
            function status($status) {
              if ($status === 'progress') {
                return array('color' =>
              'warning', 'text' => 'Proses'); } else if ($status === 'success')
              { return array('color' => 'success', 'text' => 'Sukses'); } else
              if ($status === 'cancelled') { return array('color' => 'danger',
              'text' => 'Gagal'); } } $index = 1; while ($order =
              mysqli_fetch_array($orders)) : ?>
              <tr>
                ...
                <td>
                  <span
                    class="badge text-bg-<?= status($order['status'])['color'] ?>"
                  >
                ...
                <td>
                  <div style="width: 100%">
                    ...
                    <i
                      ...
                      class="bi bi-x-circle btn-status me-3"
                      style="color: red"
                    ></i>
                    <i
                      data-id="<?= $order['id'] ?>"
                      data-status="success"
                      class="bi bi-check-circle btn-status"
                      style="color: green"
                    ></i>
                    <?php endif ?>
                  </div>
    ...
    <script>
      ...
      $(document).on("click", ".btn-status", function () {
        ...
        Swal.fire({
          title: `Yakin ingin ${
            status === "success" ? "Mensukseskan" : "Membatalkan"
          } Pesanan`,
          icon: "warning",
